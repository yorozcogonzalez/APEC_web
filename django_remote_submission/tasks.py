...
def submit_job_to_server(job_pk, password=None, public_key_filename=None, username=None,
    ...
    with wrapper.connect(password, public_key_filename):
        ...
        # Executing the propgarm on the remote server
        ...
        logs.flush()  # YOE After executed the command write stdout and stderr 
                      # to the db and delete the variables
...
#
@shared_task
def copy_file_to_server(job_pk, password=None, public_key_filename=None, username=None,
                         timeout=None, log_policy=LogPolicy.LOG_LIVE,
                         store_results=None, remote=True):

    logger.debug("copy_file_to_server: %s", locals().keys())
...
#
@shared_task
def delete_jobs_from_server(job_pk, password=None, public_key_filename=None, username=None,
                         timeout=None, log_policy=LogPolicy.LOG_LIVE,
                         store_results=None, remote=True):

    logger.debug("delete_jobs_from_serverr: %s", locals().keys())
